...
export class DataService {
  ...
  public async loadExcelData(): Promise<ProcessedData> {
    ...
    try {
      ...
      if (!fs.existsSync(excelPath)) {
        throw new Error('Excel file not found. Please ensure new_df_eval.xlsx is in the project root.');
      }
  ...
  private processRawData(rawData: any[][]): ProcessedData {
    ...
    Object.keys(processed).forEach(lob => {
      processed[lob].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    });
  ...
  private generateMockData(): ProcessedData {
    const lobs = [
      'Case Type 1', 'Case Type 2', 'Case Type 3', 'Case Type 4',
      'Case Type 5', 'Case Type 6', 'Chat Support', 'Phone Support'
    ];
    ...
    lobs.forEach(lob => {
      ...
      for (let i = 0; i < 24; i++) {
        ...
        const seasonality = Math.sin((i * 2 * Math.PI) / 12) * (baseValue * 0.1);
        const trend = i * (Math.random() * 20 - 10);
        const noise = (Math.random() - 0.5) * (baseValue * 0.05);
        
        const value = Math.max(0, Math.round(baseValue + seasonality + trend + noise));
